...
const createClient = (optns) => {
  ...
  if (recreate || !redisClient) {
    ...
    redisClient.uuid = redisClient.uuid || uuid.v1();
...
const createPublisher = (optns) => {
  ...
  if (recreate || !redisPublisher) {
    ...
    redisPublisher.uuid = redisPublisher.uuid || uuid.v1();
...
const createSubscriber = (optns) => {
  ...
  if (recreate || !redisSubscriber) {
    ...
    redisSubscriber.uuid = redisSubscriber.uuid || uuid.v1();
...
const keyFor = (...args) => {
  ...
  parts = !lodash.isEmpty(parts) ? parts : [uuid.v1()];
